...
async def handler(event):
    data = event.data.decode('utf-8')
    if data == 'sython':
        await bot.send_message(OWNER_ID, f"""**∘ بوت فرعون لتجميع النقاط واوامر اخرى 

∘ مطور ومبرمج البوت فرعون - @f_r_3_a_o_n
...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        ...
        [Button.inline('رشق مشاهدات ⟐', 'akbo')],
         ...
         [Button.inline('༺ 𝐅𝐑3𝐎𝐍 𝐁𝐎𝐓 ༻', 'button0')]
        ]
